shared_examples 'Base::render' do
  ...
  context 'Given blueprint has a Proc :datetime_format which fails to process date' do
    ...
    it('raises original error wrapped in BlueprintError from Proc') { expect{subject}.to raise_error(an_instance_of(Blueprinter::BlueprinterError).and(having_attributes({cause: an_instance_of(NoMethodError)}))) }
  end
  ...
  context 'Given blueprint refrences invalid view' do
    ...
    it('raises an BlueprinterError') {expect{subject}.to raise_error(Blueprinter::BlueprinterError, /included view 'not_existing' does not exist/)}
  end
